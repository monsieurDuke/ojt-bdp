...
public class SharedFlowImpl<T> extends AbstractSharedFlow<SharedFlowSlot> implements MutableSharedFlow<T>, CancellableFlow<T>, FusibleFlow<T> {
    ...
    public final Object awaitValue(SharedFlowSlot slot, Continuation<? super Unit> continuation) {
        ...
        synchronized (this) {
            if (tryPeekLocked(slot) < 0) {
                slot.cont = cancellableContinuationImpl2;
                slot.cont = cancellableContinuationImpl2;
            } else {
    ...
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:20:0x00bc -> B:14:0x00bf). Please submit an issue!!! */
    /*
    ...
    */
    ...
    public final Object emitSuspend(T t, Continuation<? super Unit> continuation) {
        ...
        if (emitter3 != null) {
            CancellableContinuationKt.disposeOnCancellation(cancellableContinuationImpl2, emitter3);
        }
        int i = 0;
    ...
    public final Continuation<Unit>[] findSlotsToResumeLocked(Continuation<Unit>[] resumesIn) {
        ...
        Continuation<Unit>[] continuationArr;
        SharedFlowImpl<T> sharedFlowImpl = this;
        Continuation<Unit>[] continuationArr2 = resumesIn;
        int length = resumesIn.length;
        SharedFlowImpl<T> sharedFlowImpl2 = sharedFlowImpl;
        if (AbstractSharedFlow.access$getNCollectors(sharedFlowImpl2) != 0 && (access$getSlots = AbstractSharedFlow.access$getSlots(sharedFlowImpl2)) != null) {
            int i = 0;
            ...
            while (i < length2) {
                ...
                if (abstractSharedFlowSlot != null) {
                    SharedFlowSlot sharedFlowSlot = (SharedFlowSlot) abstractSharedFlowSlot;
                    Continuation<? super Unit> continuation = sharedFlowSlot.cont;
                    if (continuation == null) {
                        continuationArr = continuationArr2;
                    } else if (sharedFlowImpl.tryPeekLocked(sharedFlowSlot) < 0) {
                        continuationArr = continuationArr2;
                    } else {
                        ...
                        continuationArr2[length] = continuation;
    ...
    private final Object[] growBuffer(Object[] curBuffer, int curSize, int newSize) {
        ...
        while (i < curSize) {
            int i2 = i;
            i++;
            SharedFlowKt.access$setBufferAt(objArr, i2 + head, SharedFlowKt.access$getBufferAt(curBuffer, i2 + head));
        }
        return objArr;
    }
