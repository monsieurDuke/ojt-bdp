...
public abstract class EventLoopImplBase extends EventLoopImplPlatform implements Delay {
        ...
        public final synchronized int scheduleTask(long now, DelayedTaskQueue delayed, EventLoopImplBase eventLoop) {
            ...
            synchronized (delayedTaskQueue) {
                ...
                if (firstImpl == null) {
                    ...
                    long j2 = j - now >= 0 ? now : j;
                    if (j2 - delayed.timeNow > 0) {
                        delayed.timeNow = j2;
                    }
                }
    ...
    private final int scheduleImpl(long now, DelayedTask delayedTask) {
        ...
        if (delayedTaskQueue == null) {
            ...
            AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(_delayed$FU, eventLoopImplBase, null, new DelayedTaskQueue(now));
    ...
    public final void mo1580dispatch(CoroutineContext context, Runnable block) {
        enqueue(block);
    }
    ...
    public void mo1581scheduleResumeAfterDelay(long timeMillis, CancellableContinuation<? super Unit> continuation) {
        long delayToNanos = EventLoop_commonKt.delayToNanos(timeMillis);
        if (delayToNanos < DurationKt.MAX_MILLIS) {
            AbstractTimeSource timeSource = AbstractTimeSourceKt.getTimeSource();
            Long valueOf = timeSource == null ? null : Long.valueOf(timeSource.nanoTime());
