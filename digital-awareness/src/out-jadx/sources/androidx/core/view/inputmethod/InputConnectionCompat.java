    ...
    public static /* synthetic */ boolean lambda$createOnCommitContentListenerUsingPerformReceiveContent$0(View view, InputContentInfoCompat inputContentInfo, int flags, Bundle opts) {
        ...
        if (Build.VERSION.SDK_INT >= 25 && (flags & 1) != 0) {
            try {
                ...
                bundle = opts == null ? new Bundle() : new Bundle(opts);
                bundle.putParcelable(EXTRA_INPUT_CONTENT_INFO, inputContentInfo2);
            } catch (Exception e) {
                Log.w(LOG_TAG, "Can't insert content from IME; requestPermission() failed", e);
                return false;
            }
        }
        return ViewCompat.performReceiveContent(view, new ContentInfoCompat.Builder(new ClipData(inputContentInfo.getDescription(), new ClipData.Item(inputContentInfo.getContentUri())), 2).setLinkUri(inputContentInfo.getLinkUri()).setExtras(bundle).build()) == null;
    }
}
