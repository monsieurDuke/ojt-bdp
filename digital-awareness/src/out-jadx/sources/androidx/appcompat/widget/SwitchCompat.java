...
public class SwitchCompat extends CompoundButton implements EmojiCompatConfigurationView {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (this.mThumbDrawable != null) {
            ...
            i4 = Math.max(0, opticalBounds.left - rect.left);
            i5 = Math.max(0, opticalBounds.right - rect.right);
        }
        if (ViewUtils.isLayoutRtl(this)) {
            i = getPaddingLeft() + i4;
            width = ((this.mSwitchWidth + i) - i4) - i5;
        } else {
            width = (getWidth() - getPaddingRight()) - i5;
            i = (width - this.mSwitchWidth) + i4 + i5;
        }
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        this.mThumbWidth = Math.max(this.mShowText ? Math.max(this.mOnLayout.getWidth(), this.mOffLayout.getWidth()) + (this.mThumbTextPadding * 2) : 0, i);
        Drawable drawable2 = this.mTrackDrawable;
        if (drawable2 != null) {
            drawable2.getPadding(rect);
