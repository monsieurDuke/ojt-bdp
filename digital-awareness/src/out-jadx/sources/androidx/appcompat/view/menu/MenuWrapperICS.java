...
public class MenuWrapperICS extends BaseMenuWrapper implements Menu {
    ...
    public int addIntentOptions(int groupId, int itemId, int order, ComponentName caller, Intent[] specifics, Intent intent, int flags, MenuItem[] outSpecificItems) {
        MenuItem[] menuItemArr = outSpecificItems != null ? new MenuItem[outSpecificItems.length] : null;
        int addIntentOptions = this.mWrappedObject.addIntentOptions(groupId, itemId, order, caller, specifics, intent, flags, menuItemArr);
        if (menuItemArr != null) {
            int length = menuItemArr.length;
            for (int i = 0; i < length; i++) {
                outSpecificItems[i] = getMenuItemWrapper(menuItemArr[i]);
            }
        }
