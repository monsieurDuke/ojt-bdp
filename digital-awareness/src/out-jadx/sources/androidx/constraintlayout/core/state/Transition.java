...
public class Transition implements TypedValues {
    ...
    public void addCustomColor(int state, String widgetId, String property, int color) {
        getWidgetState(widgetId, null, state).getFrame(state).addCustomColor(property, color);
    }

    public void addCustomFloat(int state, String widgetId, String property, float value) {
        getWidgetState(widgetId, null, state).getFrame(state).addCustomFloat(property, value);
    }
    ...
    public void addKeyPosition(String target, int frame, int type, float x, float y) {
        ...
        typedBundle.add(TypedValues.PositionType.TYPE_PERCENT_X, x);
        typedBundle.add(TypedValues.PositionType.TYPE_PERCENT_Y, y);
        getWidgetState(target, null, 0).setKeyPosition(typedBundle);
        KeyPosition keyPosition = new KeyPosition(target, frame, type, x, y);
        HashMap<String, KeyPosition> hashMap = this.keyPositions.get(Integer.valueOf(frame));
        if (hashMap == null) {
            hashMap = new HashMap<>();
    ...
    public KeyPosition findNextPosition(String target, int frameNumber) {
        ...
        while (frameNumber <= 100) {
            HashMap<String, KeyPosition> hashMap = this.keyPositions.get(Integer.valueOf(frameNumber));
    ...
    public int getNumberKeyPositions(WidgetFrame frame) {
        ...
        for (int i2 = 0; i2 <= 100; i2++) {
            HashMap<String, KeyPosition> hashMap = this.keyPositions.get(Integer.valueOf(i2));
            if (hashMap != null && hashMap.get(frame.widget.stringId) != null) {
                i++;
            }
        }
        return i;
    ...
    public void interpolate(int parentWidth, int parentHeight, float progress) {
        ...
        while (it.hasNext()) {
            this.state.get(it.next()).interpolate(parentWidth, parentHeight, progress, this);
        }
    }
    ...
    public void updateFrom(ConstraintWidgetContainer container, int state) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            getWidgetState(constraintWidget.stringId, null, state).update(constraintWidget, state);
        }
    }
}
