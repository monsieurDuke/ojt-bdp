...
public class KeyCache {
    ...
    public float getFloatValue(Object view, String type, int element) {
        ...
        float[] fArr;
        if (this.map.containsKey(view) && (hashMap = this.map.get(view)) != null && hashMap.containsKey(type) && (fArr = hashMap.get(type)) != null && fArr.length > element) {
            return fArr[element];
        }
        return Float.NaN;
    }
    ...
    public void setFloatValue(Object view, String type, int element, float value) {
        ...
        HashMap<String, float[]> hashMap2 = this.map.get(view);
        if (hashMap2 == null) {
