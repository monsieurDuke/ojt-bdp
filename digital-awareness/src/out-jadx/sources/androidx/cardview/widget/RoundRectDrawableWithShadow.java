...
class RoundRectDrawableWithShadow extends Drawable {
    ...
    private void drawShadow(Canvas canvas) {
        ...
        boolean z = this.mCardBounds.width() - (f3 * 2.0f) > 0.0f;
        boolean z2 = this.mCardBounds.height() - (f3 * 2.0f) > 0.0f;
    ...
    public float getMinHeight() {
        ...
        return (((this.mRawMaxShadowSize * SHADOW_MULTIPLIER) + this.mInsetShadow) * 2.0f) + (Math.max(f, this.mCornerRadius + this.mInsetShadow + ((f * SHADOW_MULTIPLIER) / 2.0f)) * 2.0f);
    }

    /* JADX INFO: Access modifiers changed from: package-private */
    public float getMinWidth() {
        float f = this.mRawMaxShadowSize;
        return ((this.mRawMaxShadowSize + this.mInsetShadow) * 2.0f) + (Math.max(f, this.mCornerRadius + this.mInsetShadow + (f / 2.0f)) * 2.0f);
    }
