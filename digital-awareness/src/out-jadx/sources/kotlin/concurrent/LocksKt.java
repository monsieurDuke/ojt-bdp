...
public final class LocksKt {
    ...
    private static final <T> T write(ReentrantReadWriteLock $this$write, Function0<? extends T> action) {
        ...
        Intrinsics.checkNotNullParameter(action, "action");
        ReentrantReadWriteLock.ReadLock readLock = $this$write.readLock();
        int i = 0;
        int readHoldCount = $this$write.getWriteHoldCount() == 0 ? $this$write.getReadHoldCount() : 0;
        for (int i2 = 0; i2 < readHoldCount; i2++) {
            readLock.unlock();
        }
        ReentrantReadWriteLock.WriteLock writeLock = $this$write.writeLock();
        writeLock.lock();
        try {
            return action.invoke();
        } finally {
