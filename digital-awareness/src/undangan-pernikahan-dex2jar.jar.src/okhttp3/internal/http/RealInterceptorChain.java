  ...
  implements Interceptor.Chain
{
  ...
  public final RealInterceptorChain copy$okhttp(int paramInt1, Exchange paramExchange, Request paramRequest, int paramInt2, int paramInt3, int paramInt4)
  {
    ...
    RealCall localRealCall = this.call;
    List localList = this.interceptors;
    return new RealInterceptorChain(localRealCall, localList, paramInt1, paramExchange, paramRequest, paramInt2, paramInt3, paramInt4);
  }
    ...
    throws IOException
  {
    ...
    if (i != 0)
    {
      ...
      Object localObject = this.exchange;
      if (localObject != null) {
        if (((Exchange)localObject).getFinder$okhttp().sameHostAndPort(paramRequest.url()))
        {
          if (this.calls == 1) {
            i = 1;
          } else {
          ...
          if (i == 0) {
            throw ((Throwable)new IllegalStateException(("network interceptor " + (Interceptor)this.interceptors.get(this.index - 1) + " must call proceed() exactly once").toString()));
          }
        }
        else
        {
          throw ((Throwable)new IllegalStateException(("network interceptor " + (Interceptor)this.interceptors.get(this.index - 1) + " must retain the same host and port").toString()));
        }
      }
      ...
      paramRequest = localInterceptor.intercept((Interceptor.Chain)localObject);
      if (paramRequest != null)
      {
        if (this.exchange != null)
        {
          ...
          if (i == 0) {
            throw ((Throwable)new IllegalStateException(("network interceptor " + localInterceptor + " must call proceed() exactly once").toString()));
          }
        }
        i = j;
        ...
        throw ((Throwable)new IllegalStateException(("interceptor " + localInterceptor + " returned a response with no body").toString()));
      }
      throw ((Throwable)new NullPointerException("interceptor " + localInterceptor + " returned null"));
    }
...
/* Location:              /home/icat/Documents/digital-awareness/src/undangan-pernikahan-dex2jar.jar!/okhttp3/internal/http/RealInterceptorChain.class
